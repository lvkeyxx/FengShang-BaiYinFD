...
public class ElecSettlementSQL {
    ...
    public static String getElecSettlementYearInfo(Map map) {
        ...
        if(MapUtil.valueIsNotNull(map, "CONTRACT")) {
        	sb.append(" CONTRACT in (" + MapUtil.stringValue(map, "CONTRACT") + ")");
        } else {
        	sb.append(" CONTRACT<>'" + "10" + "'");
        }
        if(MapUtil.valueIsNotNull(map, "TRAND_MONTH")) {
        	sb.append(" and substr(TRADE_MONTH, 1, 6)='" + MapUtil.stringValue(map, "TRAND_MONTH") + "'"
    ...
    public static String getElecSettlementMonInfo(Map map){
        ...
        String TRAND_MONTH = "";
        if(MapUtil.valueIsNotNull(map, "TRAND_MONTH")) {
        	TRAND_MONTH = MapUtil.stringValue(map, "TRAND_MONTH");
        }
        if(MapUtil.valueIsNotNull(map, "TRAND_YEAR")) {
        	TRAND_MONTH = MapUtil.stringValue(map, "TRAND_YEAR");
        }
        sb.append(" and substr(TRADE_MONTH, 1, 4)='" + TRAND_MONTH.substring(0, 4) + "' group by TRADE_MONTH");
        return sb.toString();
    }
